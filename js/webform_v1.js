...
Ramil_Webforms = Class.create({
	...
	test: function()
	{
		...
		for(i=0;i<this.element.childNodes.length; i++)
		{
			...
			if (this.element.childNodes[i].tagName !== undefined)
			{
				...
				if (this.options.debug == true)
					alert("Количество атрибутов = "+this.element.childNodes[i].attributes.length);
	...
	{
		if (this.options.debug == true)
			alert("Имя тэга = "+this.element.tagName.toLowerCase());
		...
		if (this.options.debug == true)
			alert("Текст старого тэга = "+this.element.outerHTML);
		...
		if (this.options.debug == true)
			alert("Текст нового тэга = "+newElement.outerHTML);
