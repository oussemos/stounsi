...
if(!window.$wf2)
{
	...
	{	
		...
		if($wf2._0){$wf2._79();$wf2._7a()}$wf2.initRepetitionBlocks();
		$wf2.initRepetitionTemplates();$wf2.initRepetitionButtons('add');
		$wf2.initRepetitionButtons('remove');$wf2.initRepetitionButtons('move-up');
		$wf2.initRepetitionButtons('move-down');$wf2._72();$wf2._73();
		if(document.addEventListener){document.addEventListener('mousedown',$wf2._49,false);
		document.addEventListener('keydown',$wf2._49,false)}else if(document.attachEvent)
		{document.attachEvent('onmousedown',$wf2._49);document.attachEvent('onkeydown',$wf2._49)}$wf2.initNonRepetitionFunctionality()},_79:function(){var select,selects=$wf2._50.apply(document.documentElement,['select','datalist']);
		for(var i=0;select=selects[i];i++){var _23=$wf2._62(select);
		if(_23&&_23.documentElement&&/:?\bselect$/i.test(_23.documentElement.nodeName)&&_23.documentElement.namespaceURI=='http://www.w3.org/1999/xhtml'){var root=_23.documentElement;if(root.getAttribute('type')!='incremental')
		{while(select.lastChild)select.removeChild(select.lastChild)}node=root.firstChild;
		while(node)
		{select.appendChild($wf2._7d(node));node=node.nextSibling}}}},
		_7a: function()
		{
			...
			for(var i=0;frm=frms[i];i++)
			{
				...
				{
					var rt; var root=_23.documentElement;
		if(root.getAttribute('type')!='incremental')frm.reset();
		var clr, clrs = root.getElementsByTagName('clear');
		for(j=0;clr=clrs[j];j++){
		if(clr.namespaceURI=='http://n.whatwg.org/formdata'&&clr.parentNode==root&&!clr.firstChild&&(rt=document.getElementById(clr.getAttribute('template')))&&rt.getAttribute('repeat')=='template'){var _4,node,next;node=rt.parentNode.firstChild;while(node){if(node.nodeType==1&&(_4=node.getAttributeNode('repeat'))&&_4.value!='template'){next=node.nextSibling;node.parentNode.removeChild(node);node=next}else node=node.nextSibling}}}
		var index,rpt, rpts=root.getElementsByTagName('repeat');
		for(j=0;rpt=rpts[j];j++){
		if(rpt.namespaceURI=='http://n.whatwg.org/formdata'&&rpt.parentNode==root&&!rpt.firstChild&&(rt=document.getElementById(rpt.getAttribute('template')))&&rt.getAttribute('repeat')=='template'&&/^-?\d+$/.test(index=rpt.getAttribute('index'))){var _3c,_4,node,next;node=rt.parentNode.firstChild;while(node){if(node.nodeType==1&&(_4=node.getAttributeNode('repeat'))&&_4.value==index){_3c=true;break}node=node.nextSibling}if(!_3c){$wf2._80.apply(rt,[null,index])}}}
		var fld,flds=root.getElementsByTagName('field');var _17=$wf2._4f.apply(frm);for(j=0;fld=flds[j];j++){var _1b=fld.getAttributeNode('index');var name=fld.getAttribute('name');
		if(!name||(_1b&&!/^\d+$/.test(_1b.value)))
			continue;
		var value='';
		for(k=0;node=fld.childNodes[k];k++)
		{
		if(node.nodeType==3||node.nodeType==4)value+=node.data;else break}var ctrl,_33=0;for(k=0;ctrl=_17[k];k++){if(ctrl.type=='image'){if(ctrl.name?(ctrl.name+'.x'==name||ctrl.name+'.y'==name):(name=='x'||name=='y')){if(!_1b||++_33-1>=_1b.value)break}}else if(ctrl.name==name){if(_1b){if(++_33-1<_1b.value)continue}else if((ctrl.type=='radio'||ctrl.type=='checkbox')&&(value&&ctrl.value!=value))continue;break}}if(ctrl.type=='file'||ctrl.type=='button'||ctrl.type=='image')continue;if(!ctrl.getAttributeNode('multiple')||!ctrl.wf2Prefilled){if(ctrl.type=='checkbox'||ctrl.type=='radio'){if(!value)ctrl.checked=false;else if(ctrl.value==value)ctrl.checked=true;else break}else if(ctrl.nodeName.toLowerCase()=='select'){ctrl.selectedIndex=-1;for(var opt,k=0;opt=ctrl.options[k];k++){if(opt.value?opt.value==value:opt.text==value){opt.selected=true;break}}}else{ctrl.value=value;$wf2._6a(ctrl);if(!ctrl.validity.valid){ctrl.value=ctrl.defaultValue;$wf2._6a(ctrl)}}ctrl.wf2Prefilled=true}else if(ctrl.getAttributeNode('multiple')){for(var opt,k=0;opt=ctrl.options[k];k++){if(!opt.selected&&(opt.value?opt.value==value:opt.text==value)){opt.selected=true;break}}}}var _17=$wf2._4f.apply(frm);for(j=0;j<_17.length;j++){}}}},_13:[],
		...
		_68: function()
		{
			...
			this.style.display='none';this.repetitionType=1;if(!this.repetitionIndex)this.repetitionIndex=0;this.repetitionTemplate=null;if(!this.repetitionBlocks)this.repetitionBlocks=[];var _attr;this.repeatStart=/^\d+$/.test(_attr=this.getAttribute('repeat-start'))?parseInt(_attr):1;
		this.repeatMin=/^\d+$/.test(_attr=this.getAttribute('repeat-min'))?parseInt(_attr):0;this.repeatMax=/^\d+$/.test(_attr=this.getAttribute('repeat-max'))?parseInt(_attr):Number.MAX_VALUE;if(!this.addRepetitionBlock)this.addRepetitionBlock=function(_c,index){return $wf2._7f.apply(this,[_c,index])};if(!this.addRepetitionBlockByIndex)this.addRepetitionBlockByIndex=this.addRepetitionBlock;var frm=this;while(frm=frm.parentNode){if(frm.nodeName.toLowerCase()=='form')break}var _templateElements;if(frm&&(_templateElements=$wf2._50.apply(this,['button','input','select','textarea','isindex'])).length){for(var el,i=0;el=_templateElements[i];i++)el.disabled=true}var _4,_f=this.parentNode.firstChild;
		while(_f&&_f!=this){if(_f.nodeType==1&&(_4=_f.getAttributeNode('repeat'))&&/^-?\d+$/.test(_4.value)&&!_f.getAttribute('repeat-template')){_f.repetitionTemplate=this;_f.setAttribute('repeat-template',this.id);this.repetitionBlocks.push(_f)}_f=_f.nextSibling}
		for(var i=0;(i<this.repeatStart||this.repetitionBlocks.length<this.repeatMin);i++)
		this.addRepetitionBlock();$wf2._13.push(this);this.wf2Initialized=true},initRepetitionTemplates:function(parentNode){var _13=$wf2._51.apply((parentNode||document.documentElement),[['*'],'repeat','template']);for(var i=0,rt;i<_13.length;i++)$wf2._68.apply(_13[i])},_65:function(){if(this.wf2Initialized)return;this.style.display='';this.repetitionType=2;var _attr;this.repetitionIndex=/^\d+$/.test(_attr=this.getAttribute('repeat'))?parseInt(_attr):0;this.repetitionBlocks=null;this.repetitionTemplate=null;var node;
		...
		if(!this.removeRepetitionBlock)
			this.removeRepetitionBlock = function()
			{
				return $wf2._81.apply(this)
			};
		if(!this.moveRepetitionBlock)
			this.moveRepetitionBlock = function(_5)
			{
		...
		repetitionButtonDefaultLabels:
			{'add':'Add','remove':'Remove','move-up':'Move-up','move-down':'Move-down'},
		_67:function(_7)
		{
			if(this.wf2Initialized)
				return;
			...
			else this.onclick=$wf2._66;this.wf2Initialized=true},initRepetitionButtons:function(_7,_2){var i;if(!_2)_2=document.documentElement;var inpts=$wf2._51.apply(_2,[['input'],'type',_7]);
		...
		if(clone.getAttributeNode('class'))
		{
			...
			clone.setAttribute('class',_className)}
		if(!/\bdisabled\b/.test(node.className))clone.disabled=false;if(node.style&&node.style.cssText){clone.style.cssText=(_1?_1(node.style.cssText):node.style.cssText)}if(node.nodeName&&node.nodeName.toLowerCase()=='label'&&node.htmlFor)clone.htmlFor=(_1?_1(node.htmlFor):node.htmlFor);if(clone.nodeName.toLowerCase()=='option')
		{
		...
		case 4:
			...
			break;
		case 8:clone=document.createComment(node.data);
			break;
		default:clone=node.cloneNode(true)}return clone},_4f:function(){var elements=[];var _39=$wf2._50.apply(this,['input','output','select','textarea','button']);for(var i=0;i<_39.length;i++){var node=_39[i].parentNode;while(node&&node.nodeType==1&&node.getAttribute('repeat')!='template')node=node.parentNode;if(!node||node.nodeType!=1)elements.push(_39[i])}return elements},
		_62: function(el)
		{
		...
		_50: function()
		{
			...
			return _1e
	},_51:function(_38,_3,_8,_35){var els,el,i,j,_1e=[];if(document.evaluate){var _3b='';if(_3)_3b='[@'+_3+(_8?(_35?'!=':'=')+'"'+_8+'"':"")+"]";var xPaths=[];for(i=0;i<_38.length;i++)xPaths.push('.//'+_38[i]+_3b);els=document.evaluate(xPaths.join('|'),this,null,7,null);for(i=0;i<els.snapshotLength;i++)_1e.push(els.snapshotItem(i))}
	else
	{
		for(i=0;i<_38.length;i++)
		{
			...
			for(j=0;el=els[j];j++)
			{
				...
				var _3e=el.getAttribute(_3);if(!_3||(thisAttrNode&&(_8===undefined||(_35?_3e!=_8:_3e==_8)))){_1e.push(el)}}}if($wf2._69)_1e.sort($wf2._69)}return _1e},_52:function(arr,item){for(var i=0;i<arr.length;i++){if(arr[i]==item)return true}return false},_53:function(el,_74){if(el.currentStyle)return el.currentStyle[_74];else if(window.getComputedStyle)return getComputedStyle(el,'').getPropertyValue(_74);else if(el.style)return el.style[_74];else return''},
		_7e:( function()
		{
			try
			{
